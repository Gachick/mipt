...
$\sum_{i=1}^{n}a_i^{2}(x) \neq 0$ в $D$.
\begin{definition}[Характеристичекая система]
\[
  \left\{\begin{aligned}
    & \dot{x_1} = a_1(x) \\ 
    & \dots  \\ 
...
\begin{proof}
  ...
  \end{gather*}
\end{proof}
\begin{definition}[Общий интеграл квазилинейного ур.]
  \[
...
\begin{theorem}[Существование и единственность решения задачи Коши]
  \phantom{.}
...
\end{proof}
\begin{definition}[Общий случай задачи Коши]
  \begin{gather*}
...
\end{definition}
\begin{theorem}[Общий случай сущ. и ед. решения]
  \phantom{.}
...
\begin{lemma}[О существовании первой вариации]
...
$J[y]\to extr$, $y\in Y$
\begin{definition}[Определение экстремума]
  $\vec{y} \in Y$ называется точкой локального min (max), если 
  \[
    \exists \epsilon >0: \: \forall y \in Y:\: 0 < \Vert y-\tilde{y} \Vert < \epsilon \ \rightarrow \ J[\tilde{y}]\le J[y] \ (\ge)
  \]
\end{definition}
\begin{lemma}[Обобщённая лемма существования первой вариации]
  \phantom{.}
  ...
  \]
\end{lemma}
\begin{lemma}[Обобщённая лемма Дюбуа-Реймона]
  \phantom{.}
...
\begin{gather*}
  ...
  J[y] \to extr \qquad y \in Y
\end{gather*}
\begin{definition}[Локальный минимум]
  \[
    \exists \epsilon > 0: \: \forall y \in Y : \: \ 0 < \Vert y - \tilde{y} \Vert < \epsilon \ \true \ J[\tilde{y}] \le J[y]
  \]
\end{definition}
